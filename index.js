...
if (symbol1===symbol2){ return true} // Використовуємо оператор '===' для порівняння двох символів
  else{return false} // Якщо два символи однакові, він поверне true, в іншому випадку - false
}
...
function getSymbolKey(symbol) {
  return Symbol.keyFor(symbol);// Викор истовуємо Symbol.keyFor(sym) для отримання ключа, який був використаний для створення символу та повертаємо його
}
...
function useSymbolToStringTag() {
  ...
  const key=Symbol('CustomObject');
  myObject[key]="3333";// Встановлюємо властивість Symbol об'єкту myObject на значення "CustomObject".
 return (key.description).toString();
